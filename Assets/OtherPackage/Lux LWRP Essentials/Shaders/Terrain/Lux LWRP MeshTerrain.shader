Shader "Lux LWRP/Terrain/Mesh Terrain"
{
    Properties
    {
        ...
        [Space(10)]
        [LuxLWRPVectorTwoDrawer] _SplatTiling("Detail Tiling (UV)", Vector) = (1,1,0,0)
...
#define Z_MESH_ATTRIBUTES COLOR
        ...
        //  defines SurfaceData, textures and the functions Alpha, SampleAlbedoAlpha, SampleNormal, SampleEmission
            ...
            VertexOutput vert (VertexInput input)
            {
                ...
                output.uv0.xy = input.texcoord; //TRANSFORM_TEX(input.texcoord, _BaseMap);
        ...
        //  Surface function which has full access to all vertex interpolators
            inline void InitializeStandardLitSurfaceData(VertexOutput IN, out SurfaceData outSurfaceData, out half3 topdownNormal)
            {
                
                topdownNormal = 0;

                float2 detailUV = IN.uv0.xy * _SplatTiling;
                half4 splatControl = 0;

                #if defined(_USEVERTEXCOLORS)
                    splatControl = IN.color;
                #else
                    splatControl.rgb = SAMPLE_TEXTURE2D(_SplatMap, sampler_SplatMap, IN.uv0.xy).rgb;
                    splatControl.a = 1.0h - splatControl.r - splatControl.g - splatControl.b;
                #endif
                

                #if defined(_TOPDOWNPROJECTION)
                    float2 uvWS = IN.positionWS.xz * _TopDownTiling;
                ...
                half3 normalTS = 0;
                    ...
                    normalTS = normalize(TransformTangentToWorld(normalTS, half3x3(IN.tangentWS.xyz, IN.bitangentWS.xyz, IN.normalWS.xyz)));
                //  We use Reoriented Normal Mapping to bring the the top down normal into world space
                    half3 n1 = /*normalize*/(IN.normalWS.xzy);
            ...
            //  Get the surface description
                InitializeStandardLitSurfaceData(IN, surfaceData, topdownNormal);
            ...
            //  Transfer all to world space 
                ...
                inputData.positionWS = IN.positionWS;
                    ...
                    #else
                        inputData.normalWS = normalize(TransformTangentToWorld(surfaceData.normalTS, half3x3(IN.tangentWS.xyz, IN.bitangentWS.xyz, IN.normalWS.xyz)));
                    #endif
                #else
                ...
                #if !SHADER_HINT_NICE_QUALITY
                    #ifdef _NORMALMAP
                        inputData.viewDirectionWS = float3(IN.normalWS.w, IN.tangentWS.w, IN.bitangentWS.w);
                    #else
...
//  Debug VR
        ...
        //half3 testf = (unity_StereoEyeIndex == 1) ? half3(0,1,0) : half3(1,1,1);
        //surfaceData.albedo *= testf; 
    #endif
