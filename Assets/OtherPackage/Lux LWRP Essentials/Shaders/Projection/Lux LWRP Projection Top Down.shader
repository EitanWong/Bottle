...
Shader "Lux LWRP/Projection/Top Down"
{
    Properties
    {
...
//  Base -----------------------------------------------------
        ...
        //  defines SurfaceData, textures and the functions Alpha, SampleAlbedoAlpha, SampleNormal, SampleEmission
            ...
            #include "Includes/Top Down LWRP Inputs.hlsl"
        ...
        //  Surface function which has full access to all vertex interpolators
            inline void InitializeStandardLitSurfaceData(VertexOutput IN, out SurfaceData outSurfaceData)
            {
                half4 albedoAlpha = SampleAlbedoAlpha(IN.uv.xy, TEXTURE2D_ARGS(_BaseMap, sampler_BaseMap));
                ...
                #if defined(_COMBINEDTEXTURE)
                    half4 combinedTextureSample = SAMPLE_TEXTURE2D(_MaskMap, sampler_MaskMap, IN.uv.xy);
                ...
                outSurfaceData.normalTS = SampleNormal(IN.uv.xy, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap), _BumpScale);

                #ifdef _NORMALMAP
                    #if defined (_MASKFROMNORMAL)
                        half4 packedNormal = SAMPLE_TEXTURE2D(_TopDownNormalMap, sampler_TopDownNormalMap, IN.uv.zw);
                        ...
                        #endif
                    #else
                        half3 topDownNormal = SampleNormal(IN.uv.zw, TEXTURE2D_ARGS(_TopDownNormalMap, sampler_TopDownNormalMap), _BumpScaleDyn);
                    #endif
                #endif
            ...
            //  Please note: outSurfaceData.normalTS will actually contain a normal in world space!
                    ...
                    //  Get per pixel worldspace normal (needed by blending)
                        float3 normalWS = TransformTangentToWorld(outSurfaceData.normalTS, half3x3(IN.tangentWS.xyz, IN.bitangentWS.xyz, IN.normalWS.xyz));
                        blendFactor = lerp(IN.normalWS.y, normalWS.y, _LowerNormalInfluence);
                    #else
                        blendFactor = IN.normalWS.y;
                    #endif
                ...
                //  Get top down projected Texture(s)
                    //float2 topDownUV = IN.positionWS.xz * _TopDownTiling + _TerrainPosition.xz;
                    half4 topDownSample = SAMPLE_TEXTURE2D(_TopDownBaseMap, sampler_TopDownBaseMap, IN.uv.zw);
                ...
                #else
                    #ifdef _NORMALMAP
                        outSurfaceData.normalTS = TransformTangentToWorld(outSurfaceData.normalTS, half3x3(IN.tangentWS.xyz, IN.bitangentWS.xyz, IN.normalWS.xyz));
                    #else
                        outSurfaceData.normalTS = IN.normalWS.xyz;
                    #endif
                #endif
...
//  Meta -----------------------------------------------------
        Pass
        {
            ...
            #pragma vertex LightweightVertexMetaCustom
            //LightweightVertexMeta
            #pragma fragment LightweightFragmentMetaCustom
            ...
            VertexOutputMeta LightweightVertexMetaCustom(Attributes input)
            {
            ...
            half4 LightweightFragmentMetaCustom(VertexOutputMeta input) : SV_Target
            {
                SurfaceData surfaceData;
    ...
    CustomEditor "LuxLWRPCustomShaderGUI"
}
